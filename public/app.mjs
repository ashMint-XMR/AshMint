...
function renderRegister(rows) { const burned = rows.reduce((total, item) => total + BigInt(item.burnAtomic || 0), 0n); const allocation = rows.reduce((total, item) => total + BigInt(item.amount || 0), 0n); const registered = config.futureToken.reference ? 1 : 0; $("#assets-count").textContent = String(registered); $("#receipt-count").textContent = String(rows.length); $("#verified-xmr").textContent = `${xmr(burned)} XMR`; $("#allocation-total").textContent = `${allocation.toLocaleString()} ${config.futureToken.symbol}`; $("#register-empty").textContent = registered ? `${config.futureToken.symbol} is registered as a future claim asset. ${rows.length ? "Receipt totals above are calculated from the accepted ledger." : "No accepted receipts yet."}` : "No assets are registered for launch yet. A token is counted only after its immutable claim reference is configured."; }
