...
function showConfig() { const live = testnet || config.launchReady; const staging = testnet || config.network === "stagenet"; $("#network-pill").textContent = testnet ? "PRACTICE MODE" : live ? (staging ? "STAGENET READY" : "MAINNET READY") : "REVIEW MODE"; $("#network-pill").classList.toggle("live", live); $("#launch-notice").innerHTML = testnet ? "<strong>Practice mode is active.</strong> Create receipts and test the complete flow. No wallet transaction or funds are used." : live ? (staging ? "<strong>Stagenet testing enabled.</strong> Test coins only. This is not a mainnet burn or token claim." : "<strong>Live verification enabled.</strong> Review every transaction in your own wallet before broadcasting.") : `<strong>Live launch setup is incomplete.</strong> ${config.blockers.join(" ")}`; $("#footer-state").textContent = testnet ? "Practice flow: active" : live ? (staging ? "Safety gate: stagenet" : "Safety gate: active") : "Safety gate: setup incomplete";
